...
const Navbar = (children) => {
    ...
    return (
        <nav className='w-full md:w-screen font-popin text-[12px] md:text-[20px] flex flex-col justify-center md:justify-between pt-2 md:pt-8 px-8 overflow-x-hidden'>
            <div className='flex'><img className='hidden md:block md:absolute top-0 w-14 md:w-32' src={logo} alt="logo" />
            <h1 className='md:hidden block text-xl font-popin font-extrabold'>Silhouette</h1>
                <button className='block md:hidden ml-auto' onClick={() => setOpen(!open)}>
                    {
                        open ? <FaRegTimesCircle size={30} /> : <FaAlignJustify size={30} />
                    }
                </button>
            </div>
            <div className='flex justify-center md:justify-end'>
                <ul className={`text-lg md:text-xl font-medium flex flex-col md:flex-row justify-center space-y-4 md:space-y-0 md:justify-items-end md:items-end items-center space-x-2 md:space-x-6 transition-all duration-1000 md:flex ${open ? "block" : "hidden"}`}>
                    <li>
                    ...
                    <li className="flex flex-col md:flex-row text-center space-x-1 space-y-2 md:space-y-0">
