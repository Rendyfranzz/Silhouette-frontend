            ...
            <div className='flex justify-center md:justify-end'>
                <ul className={`text-black text-lg md:text-xl font-bold flex flex-col md:flex-row justify-center space-y-4 md:space-y-0 md:justify-items-end md:items-end items-center space-x-2 md:space-x-6 transition-all duration-1000 md:flex ${open ? "" : "hidden"}`}>
                    ...
                    <li className="flex flex-col md:flex-row text-center justify-center items-center space-x-1 space-y-2 md:space-y-0">
                        ...
                        {user ? <><NavLink to={`/userdetail/${user.uuid}`} className={({ isActive }) => isActive ? "text-cyan-500" : ""}>{user.name}</NavLink> <button onClick={out}>logOut</button></> : <><FaUserCircle size={20} /> <NavLink to='/login' className="text-black no-underline">Login</NavLink></>
                        }
                    </li>
                </ul>
                <Notify />
